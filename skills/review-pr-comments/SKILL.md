...
1. **Code fixes**: read the file and surrounding context. Apply the minimum surgical change. When addressing a valid comment, consider why it slipped through — then generalize: scan all code being introduced in this PR for the same class of issue and fix every instance in the same pass. Do not patch only the reported line.
   ...
   **Every code fix must be accompanied by a test, or an explicit written justification for why a test is not needed.** "Untestable in jsdom / a SwiftUI view body / a lifecycle closure" is not by itself an acceptable justification — most decision logic embedded in a framework callback (which branch to take, which value to compute) can be extracted into a plain function, object, or module with no framework dependency, called from a thin adapter closure. Before citing untestability, attempt that extraction: move the decision into a pure function that takes the same inputs the closure receives and returns the same result, call it from the closure, and test the extracted function directly. Only skip a test if, after attempting extraction, the remaining logic still requires a live framework runtime (real DOM/window timing, real OS lifecycle callbacks, live rendering) that cannot be driven synchronously — or if an existing test already covers the corrected behavior. If you skip a test, state in the action plan and commit message specifically what could not be extracted and why, not just "untestable in jsdom" or the equivalent for the current framework. Do not silently omit tests.
