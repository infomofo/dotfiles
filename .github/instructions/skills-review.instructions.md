...
- When a skill step references a bundled script via a relative markdown link, that link is the intended invocation instruction for the AI agent reading the skill. Do not flag the absence of an explicit shell command such as `python3 /path/to/script.py` — skill files are read by AI agents, not shell users. A markdown link to a script is not ambiguous; it is the correct and complete instruction.
